...
export default function TenantAdminsModal({ isOpen, onClose, tenant }) {
  ...
  const handleResetPassword = async (adminId) => {
    ...
    setError(null);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || (window.location.protocol + '//' + window.location.hostname + ':5001')}/api/tenants/${tenant.id}/admins/${adminId}/password`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-5 flex items-center justify-between border-b border-slate-100 dark:border-slate-700/50 bg-white">
          <div className="flex items-center gap-4">
            {tenant.logo_url ? (
              <img src={tenant.logo_url} alt={tenant.nume} className="w-10 h-10 rounded-full object-cover border border-slate-200 dark:border-slate-700" />
            ) : (
              <div 
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-800 dark:text-white leading-tight">{tenant.nume}</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400 font-medium leading-tight">Admini locali</p>
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50 dark:bg-slate-800/50/50">
          ...
          {error && (
            <div className="mb-6 p-4 rounded-lg bg-red-50 border border-red-200 text-sm font-bold text-red-600">
              {error}
            </div>
          )}
          ...
          {/* Add Admin Sub-form */}
          {showAddForm && (
            <form onSubmit={handleAddSubmit} className="mb-6 bg-white dark:bg-slate-900 p-5 rounded-lg border border-primary-100 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1">Parolă de acces *</label>
                  <div className="relative">
                ...
                <div className="pt-2 flex justify-end gap-2">
                  <button 
                    ...
                    className="px-4 h-9 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-sm font-bold transition-colors"
                  >
                    Anulează
                  </button>
                  <button 
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-base font-bold text-slate-800 dark:text-white mb-1">Niciun admin local</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-8">Adaugă primul administrator pentru această companie.</p>
            </div>
          ) : (
            <div className="space-y-3 mb-8">
              {admins.map(admin => (
                <div key={admin.id} className="bg-white dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-3">
          ...
          {/* Info Box */}
          <div className="bg-blue-50/80 border border-blue-100 p-4 rounded-lg flex gap-3 mt-4">
