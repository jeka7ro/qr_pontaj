...
export default function QrSelector({ tenant, themeColor }) {
  ...
  const [portalSettings, setPortalSettings] = useState({
    portal_bg_image_url: tenant?.portal_bg_image_url || '',
    portal_bg_color: tenant?.portal_bg_color || '#000000'
  });
  ...
  const savePortalSettings = async () => {
    ...
    setPortalMsg(null);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || (window.location.protocol + '//' + window.location.hostname + ':5001')}/api/tenants/${tenant.id}/portal-settings`, {
        method: 'PUT',
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [kRes, lRes] = await Promise.all([
        fetch(`${apiUrl}/api/tenants/${tenant.id}/kiosks`, { cache: 'no-store' }),
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || (window.location.protocol + '//' + window.location.hostname + ':5001')}/api/tenants/${tenant.id}/kiosks/${deleteConfirmId}`, {
        method: 'DELETE',
        cache: 'no-store'
      });
  ...
  const openKiosk = (kioskId, mode = null) => {
    let url = `/kiosk/${tenant.id}/${kioskId}`;
  ...
  return (
    <div className="p-6 max-w-7xl space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 md:p-8 shadow-sm border border-slate-200 dark:border-slate-800 mb-8">
        ...
        {portalMsg && (
          <div className={`p-4 rounded-lg mb-6 flex items-start gap-3 ${portalMsg.type === 'error' ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>
            <AlertCircle size={20} className="shrink-0 mt-0.5" />
            <p className="font-medium text-sm">{portalMsg.text}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input 
              ...
              onChange={e => setPortalSettings({...portalSettings, portal_bg_image_url: e.target.value})}
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input 
                type="color"
                value={portalSettings.portal_bg_color || '#000000'}
                onChange={e => setPortalSettings({...portalSettings, portal_bg_color: e.target.value})}
                className="w-12 h-12 rounded-full cursor-pointer border-0"
                style={{ 
                  border: `2px solid ${portalSettings.portal_bg_color || '#000000'}`, 
                  backgroundColor: 'white', 
                  padding: '3px' 
                }}
              />
              <button 
                onClick={() => setPortalSettings({...portalSettings, portal_bg_color: '#000000'})}
                className="text-xs font-bold text-slate-500 hover:text-slate-700 bg-slate-100 dark:bg-slate-800 px-3 py-2 rounded-full"
              >
                Reset
              </button>
            </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="w-12 h-12 rounded-full cursor-pointer border-0 bg-transparent"
                        style={{ 
                          border: `2px solid ${formData.kiosk_timer_color || '#3b82f6'}`, 
                          backgroundColor: 'white', 
                          padding: '3px' 
                        }}
                      />
                    ...
                    {formData.kiosk_show_timer_bg !== false && (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="w-12 h-12 rounded-full cursor-pointer border-0 bg-transparent"
                          style={{ 
                            border: `2px solid ${formData.kiosk_timer_bg_color || '#000000'}`, 
                            backgroundColor: 'white', 
                            padding: '3px' 
                          }}
                        />
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="w-12 h-12 rounded-full cursor-pointer border-0 bg-transparent"
                        style={{ 
                          border: `2px solid ${formData.kiosk_bg_color || '#020617'}`, 
                          backgroundColor: 'white', 
                          padding: '3px' 
                        }}
                      />
                    ...
                    {formData.kiosk_show_logo_bg !== false && (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="w-12 h-12 rounded-full cursor-pointer border-0 bg-transparent"
                          style={{ 
                            border: `2px solid ${formData.kiosk_logo_bg || '#0f172a'}`, 
                            backgroundColor: 'white', 
                            padding: '3px' 
                          }}
                        />
